...
协议基线:`shared/contracts/http/openapi.yaml` + `shared/contracts/realtime/event.schema.json`(v0.9.0)。
...
| 页面 | ✅ | `/login`(登录/注册 tab,RHF+Zod)、`/rooms`(我的房间+公开大厅+创建)、`/rooms/:roomId`(消息/成员/接入/房主设置；公开免邀请码加入) |
| 房间治理 | ✅ | 私有/公开切换、改名、解散、踢人二次确认；被踢或解散后实时退出，公开房间可发现和直接加入 |
...
| 测试 | ✅ | 覆盖安装命令、幂等键、message/member store、presence、RealTimeClient 连接/治理事件/心跳/断线重连 |
